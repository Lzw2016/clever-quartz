 ...
 */
@Slf4j
public class TriggerLogPlugin implements SchedulerPlugin, TriggerListener {
    ...
    public void initialize(String name, Scheduler scheduler, ClassLoadHelper loadHelper) throws SchedulerException {
        qrtzTriggerLogService = SpringContextHolder.getBean(TriggerLogService.class);
        if (qrtzTriggerLogService != null) {
            log.info("### [TriggerLogPlugin]QrtzTriggerLogService注入成功");
        } else {
            RuntimeException exception = new RuntimeException("Quartz插件[TriggerLogPlugin]初始化失败，QrtzTriggerLogService注入失败");
        ...
        log.info("### Quartz插件[TriggerLogPlugin]初始化成功");
    }
    ...
    public void triggerComplete(Trigger trigger, JobExecutionContext context, Trigger.CompletedExecutionInstruction triggerInstructionCode) {
        ...
        if (id == null) {
            log.warn("### trigger.getJobDataMap().get(jobDataMapKey) 返回空值-triggerComplete");
            return;
        }
        QrtzTriggerLog qrtzTriggerLog = new QrtzTriggerLog();
