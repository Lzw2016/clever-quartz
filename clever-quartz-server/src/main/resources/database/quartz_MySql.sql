...
(
  ...
  COMMENT 'Job描述， .withDescription()方法传入的string',
  ...
  COMMENT 'Scheduler实例发生故障时，故障恢复节点会检测故障的Scheduler正在调度的任务是否需要recovery(复苏)，如果需要会添加一个只执行一次的simple trigger重新触发',
  JOB_DATA          BLOB         NULL
...
(
  ...
  COMMENT 'Trigger group名称',
  JOB_NAME       VARCHAR(200) NOT NULL
  COMMENT 'Job key',
  JOB_GROUP      VARCHAR(200) NOT NULL
  COMMENT 'Job group 名称',
  DESCRIPTION    VARCHAR(250) NULL
  COMMENT 'Trigger 描述， .withDescription()方法传入的string',
  ...
  COMMENT 'Trigger状态，PAUSED_BLOCKED:停止_阻塞; PAUSED:停止; WAITING:等待执行; ACQUIRED:已获得; EXECUTING:执行中; COMPLETE:完成; BLOCKED:阻塞; ERROR:错误; DELETED:已删除',
  ...
  COMMENT 'Trigger关联的Calendar name',
...
(
  ...
  COMMENT '执行节点IP,可能有多个(‘;’分隔)',
  status          CHAR(1)      NOT NULL
  COMMENT '任务执行状态(0:成功;1:失败,2:未执行完成)',
  exception_info  MEDIUMTEXT COMMENT '异常信息',
  is_veto         CHAR(1)      NOT NULL
  COMMENT '是否被否决（0：否；1：是）',
...
(
  ...
  COMMENT '触发次数',
  ip_address               VARCHAR(200) NOT NULL
  COMMENT '触发节点IP,可能有多个(‘;’分隔)',
  mis_fired                CHAR(1)      NOT NULL
  COMMENT '是否错过了触发（0：否；1：是）',
  before_job_data          MEDIUMTEXT COMMENT '执行前的JobDataMap数据',
  after_job_data           MEDIUMTEXT COMMENT '执行后的JobDataMap数据',
